...
exports.generateYieldAnalysis = generateYieldAnalysis;
exports.analyzeWalletIntelligence = analyzeWalletIntelligence;
exports.analyzeSocialSentiment = analyzeSocialSentiment;
exports.generateMarketBrief = generateMarketBrief;
const groq_sdk_1 = __importDefault(require("groq-sdk"));
const logger_1 = require("../lib/logger");
const schemas_1 = require("../lib/schemas");
const groq = new groq_sdk_1.default({ apiKey: process.env.GROQ_API_KEY });
const MODEL = process.env.GROQ_MODEL ?? 'llama-3.3-70b-versatile';
const RISK_MODE_LABELS = {
    0: 'conservative',
    1: 'moderate',
    2: 'aggressive',
};
...
async function withRetry(fn, retries = 2) {
    for (let attempt = 0; attempt <= retries; attempt++) {
        try {
            ...
            logger_1.logger.warn('Groq call failed, retrying', { attempt, delay });
...
async function generateYieldAnalysis(input) {
...
Respond ONLY with a JSON object:
{
  ...
  "risk_note": "<1 sentence caveat specific to these assets and risk mode>"
    ...
    return withRetry(async () => {
        ...
        logger_1.logger.info('Groq yield analysis raw', { raw: raw.slice(0, 200) });
        const parsed = extractJson(raw);
        return schemas_1.AIRecommendationSchema.parse(parsed);
    });
}
...
async function analyzeWalletIntelligence(address) {
...
Return ONLY a JSON object with this exact schema:
{
  ...
  "source": "groq-analysis"
    ...
    return withRetry(async () => {
        ...
        const raw = completion.choices[0]?.message?.content ?? '';
        const parsed = extractJson(raw);
        return schemas_1.NansenResponseSchema.parse(parsed);
    });
}
...
async function analyzeSocialSentiment(query) {
...
Return ONLY a JSON object with this exact schema:
{
  ...
  "source": "groq-analysis"
    ...
    return withRetry(async () => {
        ...
        const raw = completion.choices[0]?.message?.content ?? '';
        const parsed = extractJson(raw);
        return schemas_1.ElfaResponseSchema.parse(parsed);
    });
}
async function generateMarketBrief(input) {
...
Respond ONLY with the JSON object.`;
    return withRetry(async () => {
        ...
        const raw = completion.choices[0]?.message?.content ?? '';
        const parsed = extractJson(raw);
        return schemas_1.MarketBriefSchema.parse(parsed);
    });
}
